...
class GoodsSKU(BaseModel):
    ...
    status = models.SmallIntegerField(verbose_name="商品状态", default=1, choices=STATUS_CHOICES)
    ...
    def __str__(self):
        ...
        return "%s-%s-%s(%s)" % (self.type.name, self.goods_spu.name, self.id, self.name)
...
class IndexTypeGoodsBanner(BaseModel):
    ...
    def __str__(self):
        ...
        return "%s(%s)-%s-%s" % (self.sku.id, self.sku.name, self.get_display_type_display(), self.index)
